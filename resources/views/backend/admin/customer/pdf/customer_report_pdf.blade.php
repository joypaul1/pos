  ...
  <div class="container">
    ...
    <div class="row">
      <div class="col col-sm-12 text-center">
        <h4 style="font-weight: bold"><u>CUSTOMER REPORT</u></h4>
      </div>
    </div>
    <div class="row">
      <div class="col-sm-12">
        <table class="table table-sm table-bordered">
          ...
          <tbody>
            ...
            <tr>
              <td>{{$key+1}}</td>
              <td>{{$value->name}},{{$value->mobile}} ({{$value->address}})</td>
              <td>{{$value->total_amount}}</td>
              <td>{{$value->payment}}</td>
              <td>{{$value->due}}</td>
              @php
                $total_sum += $value->total_amount;
                $paid_sum += $value->payment;
                $due_sum += $value->due;
              @endphp
            </tr>
            @endforeach
            <tr>
              <td colspan="2" style="text-align: right;"><p style="font-weight: bold;">Grand Total</p></td>
              <td><p style="font-weight: bold;">{{$total_sum}} TK</p></td>
              <td><p style="font-weight: bold;">{{$paid_sum}} TK</p></td>
              <td><p style="font-weight: bold;">{{$due_sum}} TK</p></td>
            </tr>
          </tbody>
        </table>
          @php
