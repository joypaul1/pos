...
<html>
<head>
  <title> কারেন্ট ব্যাল্যান্স রিপোর্ট</title>
...
<body>
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-12">
        <p style="text-align: center;margin-bottom: 0px;font-weight: bold;font-size: 20px;"><u>কারেন্ট ব্যাল্যান্স রিপোর্ট ({{date('d-m-Y',strtotime($start_date))}} - {{date('d-m-Y',strtotime($end_date))}})</u></p>
      	<hr>
      </div>
      <div class="col-md-12">
        ইনকামের তালিকা
        <table class="table table-bordered table-striped dt-responsive nowrap" style="width: 100%">
            ...
            <tbody>
              @php
                $total_sum = 0;
              @endphp
              @foreach($allIncomes as $key => $invoice)
              <tr>
                <td>{{$key+1}}</td>
                <td>
                  {{$invoice['payment']['customer']['name']}} - {{$invoice['payment']['customer']['mobile']}} ({{$invoice['payment']['customer']['address']}})
                </td>
                <td>{{date('d-m-Y',strtotime($invoice->date))}}</td>
                <td># {{$invoice->invoice_no}}</td>
                <td>{{$invoice->description}}</td>
                <td>{{$invoice['payment']['paid_amount']}} টাকা</td>
                @php
              ...
              @endforeach
              <tr>
                <td class="text-right" colspan="5"><strong>সর্বমোট</strong></td>
                <td><strong>{{$total_sum}} টাকা</strong></td>
              </tr>
            </tbody>
        </table>
        ব্যয়ের তালিকা
        <table class="table table-bordered table-striped dt-responsive nowrap" style="width: 100%">
            ...
            <tbody>
              ...
              @endforeach
              <tr>
                <td class="text-right" colspan="3"><strong>সর্বমোট</strong></td>
                <td><strong>{{$total_expanse}} টাকা</strong></td>
              </tr>
            </tbody>
        </table>
        ...
        <!-- <table class="table table-bordered table-striped dt-responsive nowrap" style="width: 100%">
          <tbody>
            <tr>
              <td class="text-right"><p style="font-size: 20px; font-weight: bold;">কারেন্ট ব্যাল্যান্স</p></td>
              <td><p style="font-size: 20px; font-weight: bold;">{{$total_sum-$total_expanse}} টাকা</p></td>
            </tr>
          </tbody>
        </table> -->
