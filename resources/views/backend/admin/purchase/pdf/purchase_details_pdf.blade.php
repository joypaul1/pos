  ...
  <div class="container">
    <div class="row">
      <table style="width: 100%">
        <tbody>
          <tr>
            <td style="width: 25%" class="text-center">Memo No: # {{$purchase['purchase']['purchase_no']}}</td>
            ...
            <td class="text-center" style="width: 25%">Date: {{date('d-m-Y',strtotime($purchase['purchase']['date']))}}</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="row">
      <div class="col-md-12">
        <table class="table">
          <tbody>
            <tr>
              <td width="100%">Supplier Name : {{$purchase['supplier']['name']}}</td>
            </tr>
            <tr>
              <td width="100%">Mobile No : {{$purchase['supplier']['mobile']}}</td>
            </tr>
            <tr>
              <td width="100%">Address : {{$purchase['supplier']['address']}}</td>
            </tr>
            <tr>
              <td width="100%">Description : {{$purchase['purchase']['description']}}</td>
            </tr>
          </tbody>
        </table>
      </div>
      <div class="col-sm-12">
        <table class="table table-sm table-bordered">
          ...
          <tbody>
            <tr>
              <td colspan="3">{{$purchase->total_amount}} TK</td>
              <td colspan="3">{{$purchase->paid_amount}} TK</td>
            ...
            @php
              $total_paid_sum = 0;
              $purchase_payment_details = App\Model\PurchasePaymentDetail::where('purchase_id',$purchase->purchase_id)->get();
            @endphp
            @foreach($purchase_payment_details as $key2 => $payment_details)
            <tr>
              <td>{{$key2+1}}</td>
          ...
          <tbody>
            <tr>
              ...
              <td style="width: 21%; text-align: center;">
                <p style="text-align: center;">{{@$purchase['purchase']['user']['name']}}</p>
              </td>
            </tr>
          </tbody>
        </table>
