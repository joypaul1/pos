...
<html>
<head>
  <title> তারিখ অনুযায়ী ডেইলি পেইড রিপোর্ট</title>
...
<body>
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-12">
        <p style="text-align: center;margin-bottom: 0px;font-weight: bold;font-size: 20px;"><u>তারিখ অনুযায়ী ডেইলি পেইড রিপোর্ট ({{date('d-m-Y',strtotime($start_date))}} - {{date('d-m-Y',strtotime($end_date))}})</u></p>
        <hr>
      </div>
      <div class="col-md-12">
        <table class="table table-bordered table-striped dt-responsive nowrap" style="width: 100%">
            ...
            <tbody>
              @php
                $total_sum = 0;
              @endphp
              @foreach($allPaid as $key => $paid)
              <tr>
                <td>{{$key+1}}</td>
                <td>
                  {{$paid['invoice']['payment']['customer']['name']}} - {{$paid['invoice']['payment']['customer']['mobile']}} ({{$paid['invoice']['payment']['customer']['address']}})
                </td>
                <td>{{date('d-m-Y',strtotime($paid->date))}}</td>
                <td># {{$paid['invoice']['invoice_no']}}</td>
                <td>{{$paid['invoice']['description']}}</td>
                <td>{{$paid->current_paid_amount}} টাকা</td>
                @php
                $total_sum += $paid->current_paid_amount;
                @endphp
              </tr>
              @endforeach
              <tr>
